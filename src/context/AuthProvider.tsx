...
async function updateToken(token: string): Promise<User | null> {
  try {
    ...
    await SecureStore.setItemAsync('token_map', JSON.stringify(parsedToken));
...
async function verifyToken(token: string): Promise<User | null> {
  ...
  async function login(username: string, password: string): Promise<boolean> {
    try {
      ...
      await SecureStore.setItemAsync('token_map', tokenString);
      console.log('makina')
      console.log(response.data)
  ...
  async function logout() {
    try {
      await SecureStore.deleteItemAsync('token_map');
      setUser(null);
    } catch (error) {
